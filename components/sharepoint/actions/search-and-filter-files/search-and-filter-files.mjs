...
export default {
  ...
  props: {
    sharepoint,
    siteId: {
      propDefinition: [sharepoint, "siteId"],
    },
    listId: {
      propDefinition: [
        sharepoint,
  ...
  async run({ $ }) {
    ...
    const expand = expandFields ? "fields" : undefined;
    ...
    for await (const item of iterator) {
      items.push({
        id: item.id,
        name: item.fields?.FileLeafRef || item.name || item.displayName,
